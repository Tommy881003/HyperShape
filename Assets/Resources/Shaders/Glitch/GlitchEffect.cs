...
public class GlitchEffect : MonoBehaviour
{
    ...
    private float realFlickIntensity;
	private Material _material;

	void Start()
	{
		_material = new Material(Shader);
        realFlickIntensity = 9f * flickIntensity + 1f;
	...
	void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		...
		{
			_material.SetFloat("filterRadius", realFlickIntensity * Random.Range(-3f, 3f) * colorIntensity);
			_material.SetVector("direction", Quaternion.AngleAxis(Random.Range(0, 360) * colorIntensity, Vector3.forward) * Vector4.one);
			flicker = 0;
			_flickerTime = Random.value / realFlickIntensity;
		}
		...
		if (_glitchup > _glitchupTime)
		{
			if (Random.value < 0.1f * flipIntensity)
				_material.SetFloat("flip_up", realFlickIntensity * Random.Range(0, 1f) * flipIntensity);
			else
				_material.SetFloat("flip_up", 0);

			_glitchup = 0;
			_glitchupTime = Random.value / (10f * realFlickIntensity);
		}
		...
		if (_glitchdown > _glitchdownTime)
		{
			...
			_glitchdown = 0;
			_glitchdownTime = Random.value / (10f * realFlickIntensity);
		}
		...
		{
			_material.SetFloat("displace", realFlickIntensity * Random.value * intensity);
